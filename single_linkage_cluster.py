...
import re
import subprocess
import warnings # also import exceptions?
import csv
...
class Contig_Cluster(object):
        ...
        if self.has_spades() == True:
            ...
            for node in self.nodes:
                if spadespattern.match(node):
                    ...
                    warnings.warn('Some contigs in this cluster were not generated by Spades. Non-Spades contig names not supported yet')
            self.av_cov = int(cov_total / spades_seqs)
            self.av_length = int(length_total / spades_seqs)
        else:
            warnings.warn('Non-Spades contig names not supported yet')
    ...
    def retrieve_seqs(self, assembly_dir, repseq = False):
        ...
        for link in nodes:
            node, assembly = link
            f = open("/".join([assembly_dir, "fr."+ assembly]))
            for i in f:
                if i.find(node.replace('>', '')) != -1:
